...
def process_documents(excelpath, wordpath, komitmen_TKDN, dirname):
    ...
    if uraian_row_index is None or uraian_column_index is None:
        ...
        for index, header in enumerate(table_header):
            ...
            second_row_cells[index].paragraphs[0].runs[0].font.bold = False
        ...
        for row_index, row_data in enumerate([['',
                ...
                format_percentage(sheet_calc.cell(row=row_index, column=8).value*100), ''] for row_index in all_rows_in_table], start=2):
        ...
        total_row.cells[2].text = format_percentage(format_percentage_in_float(komitmen_TKDN))
        ...
        # Calculate the sum of all numbers in the fifth column and set it as the value of the fifth cell
        selisihTKDN_SA_Verif = format_percentage(total_calculated_Verifvalue - format_percentage_in_float(komitmen_TKDN))
