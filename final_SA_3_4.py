...
def process_documents(excelpath, wordpath, dirname):
    ...
    if uraian_row_index is None or uraian_column_index is None:
        print("The cell with value 'Uraian' was not found in the sheet 'Form 3.4'.")
    else:
        # Extract all rows below the "Uraian" cell and in the same column, excluding empty cells
        for row in sheet.iter_rows(min_row=uraian_row_index+4, min_col=uraian_column_index, max_col=uraian_column_index, values_only=False):
            if row[0].value is not None and row[0].value != "None":  # Check if the cell is not empty
                all_rows_in_table.append(row[0].row)
        ...
        for row in sheet.iter_rows(min_row=17, max_row=34, min_col=11, max_col=11):
            for cell in row:
        ...
        table_header = ['No.','Uraian', 'Nama Perusahaan', 'TKDN \n (%)', 'Struktur Biaya (%)']
        ...
        for index, header in enumerate(table_header):
            ...
            header_cells[index].paragraphs[0].runs[0].font.bold = False
        ...
        format_percentage(sheet.cell(row=row_index, column=5).value) if isinstance(sheet.cell(row=row_index, column=5).value, (int, float)) else "0,00"] for row_index in all_rows_in_table], start=2):
            row_cells = table.add_row().cells
            # print(row_data)
            for index, cell in enumerate(row_cells):
                if index == 0:  # Add numbering to the first column
                    cell.text = str(row_index - 1)
                elif index == 4:  # Insert values from baris_struktur_biaya into the "Struktur Biaya" column
        ...
        for idx, cell in enumerate(table.columns[4].cells):
        ...
        merged_cells = total_row.cells[1].merge(total_row.cells[3])
        ...
        total_row.cells[4].text = str(total_struktur_biaya_formatted)
